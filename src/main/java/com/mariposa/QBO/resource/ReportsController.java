...
public class ReportsController {
	...
	public String callReportsConcept(HttpSession session) {
		...
		try {
			...
			// get ReportService
			ReportService service = factory.getReportService(realmId, accessToken);

			/*
			 * Read default profit and Loss report: Default date used: This fiscal year to
			 * date Deafult accounting method: Defined in
			 * Preferences.ReportPrefs.ReportBasis. The two accepted values are "Accural"
			 * and "Cash" Default includes data for all customers
			 */
			Report defaultPnLReport = service.executeReport(ReportName.PROFITANDLOSS.toString());

			/*
			 * Read default Balance sheet report: Default date used: This fiscal year to
			 * date Deafult accounting method: Defined in
			 * Preferences.ReportPrefs.ReportBasis. The two accepted values are "Accural"
			 * and "Cash" Default includes data for all customers Default it is summarized
			 * by Total
			 */
			Report defaultBalanceSheet = service.executeReport(ReportName.BALANCESHEET.toString());

			/*
			 * report for given start and end date set start_date and end_date properties in
			 * the ReportService instance with the date range in yyyy-mm-dd format
			 */
			...
			/*
			 * Year End Balance Sheet report summarized by Customer set the customer
			 * property to the customer.Id and set summarize_column_by property to
			 * "Customers" You can also set customer property with multiple customer ids
			 * comma seperated. You can summarize by the following:Total, Customers,
			 * Vendors, Classes, Departments, Employees, ProductsAndServices by setting the
			 * summarize_column_by property
			 */
